...
def Friends(request):
	...
	if request.method == 'POST':
		...
		print(friend)
		if friend != '':
			friends = UserContact.objects.filter(Q(phone_number__icontains = friend)|
				Q(user__username__icontains = friend))
...
def ChatRoom(request, pk):
	...
	try:
		...
		Q(request_receiver = request.user, request_sender = user)).exists():
			...
			Q(receiver = request.user, sender = user)).order_by("date")
			# error = False
...
def PostComment(request,pk):
	...
	if post.comment:
		...
		posts_and_comment_id5 = zip(post_and_comment_id[:5], post.comment.all().order_by('-id')[:5])
		...
		context = {
					...
					'posts_and_comment_id5':posts_and_comment_id5,
